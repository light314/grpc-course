...
public class GreetingClient {
    ...
    private static void doLongGreet(ManagedChannel channel) throws InterruptedException {
        ...
        Collections.addAll(names, "Vinodh", "Rohini", "Yashna");
    ...
    private static void doGreetEveryone(ManagedChannel channel) throws InterruptedException {
        ...
        Arrays.asList("Vinodh", "Rohini", "Yashna").forEach(name ->{
            stream.onNext(GreetingRequest.newBuilder().setFirstName(name).build());
        });
        stream.onCompleted();
        latch.await(3, TimeUnit.SECONDS);
    ...
    public static void doGreetWithDeadline(ManagedChannel channel) {
        ...
        GreetingServiceGrpc.GreetingServiceBlockingStub stub = GreetingServiceGrpc.newBlockingStub(channel);
        GreetingResponse response = stub.withDeadline(Deadline.after(3, TimeUnit.SECONDS)).greetWithDeadline(GreetingRequest.newBuilder().setFirstName("Vinodh").build());
        System.out.println("Greetig within Deadline:"+response.getResult());
        try{
            response= stub.withDeadline(Deadline.after(100, TimeUnit.MILLISECONDS)).greetWithDeadline(GreetingRequest.newBuilder().setFirstName("Vinodh").build());
